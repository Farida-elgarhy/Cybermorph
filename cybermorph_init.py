...
FIXED: Logger initialized before use
"""
...
class CyberMorphInitializer:
    ...
    def setup_logging(self) -> logging.Logger:
        """Setup logging configuration - MUST BE FIRST"""
    ...
    def validate_config(self):
        ...
        required_keys = ['target', 'replica', 'discovery', 'sanitization']
    ...
    def validate_ssh_credentials(self, target: Dict[str, Any]):
        ...
        try:
            if 'key_file' in target and target['key_file']:
                ssh.connect(
            ...
            elif 'password' in target and target['password']:
                ssh.connect(
                    hostname=target['host'],
                    port=target.get('port', 22),
                    username=target['username'],
                ...
                raise ValueError("No SSH credentials provided (key_file or password)")
    ...
    def detect_target_os(self):
        ...
        if self.config['target']['type'] == 'linux':
            ...
            try:
                ...
                self.target_os = self.config['target']['type']
    ...
    def validate_cloud_credentials(self, cloud_config: Dict[str, Any]):
        ...
        provider = cloud_config.get('provider', 'docker')
        
        if provider == 'docker':
            # Docker doesn't need credentials
            self.logger.info("Using Docker for replica environment")
            return
        ...
        if provider == 'aws':
            try:
                ...
                self.logger.warning(f"AWS credentials may be invalid: {str(e)}")
            ...
            try:
                ...
                self.logger.warning(f"Azure credentials may be invalid: {str(e)}")
            ...
            try:
                ...
                self.logger.warning(f"GCP credentials may be invalid: {str(e)}")
    ...
    def validate_on_premise_credentials(self, on_premise_config: Dict[str, Any]):
        ...
        self.logger.info("On-premise replica environment configured")
    ...
    def check_disk_space(self):
        ...
        try:
            ...
            # Estimate replica size (rough estimate: 100GB minimum)
            required_space = 100 * 1024 * 1024 * 1024  # 100GB
            
            if free < required_space:
                self.logger.warning(f"Low disk space. Required: {required_space}, Available: {free}")
            else:
                self.logger.info(f"Disk space available: {free / (1024**3):.2f} GB")
        except Exception as e:
